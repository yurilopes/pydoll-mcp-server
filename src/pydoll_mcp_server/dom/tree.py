...
async def build_page_tree(
    ...
    if cache_elements:
            ...
            for node in nodes:
                ...
                entry = ElementCacheEntry(
                    ...
                    text_summary=(node.get('text', '') or '')[:100],
                    bounding_box=node.get('bounds') or {},
                    selector_hint=_build_selector_hint(node),
                    xpath_hint=_build_xpath_hint(node),
                )
                node['actionable'] = actionable
                node['resolution_confidence'] = confidence
                node['selector_hint'] = entry.selector_hint
                node['xpath_hint'] = entry.xpath_hint
                cache.store(entry)
    ...
    return {
        ...
        'nodes': nodes,
        'count': len(nodes),
        'truncated': raw.get('count', 0) >= max_nodes,
    }


def _assess_actionable(node: dict) -> tuple[bool, str]:
    tag = node.get('tag', '')
...
def _build_selector_hint(node: dict) -> str:
    tag = node.get('tag', '')
...
def _build_xpath_hint(node: dict) -> str:
    tag = node.get('tag', '')
    if tag in ('#text', '#document', 'html', 'head', 'body'):
        return ''
    attrs = node.get('attrs', {})
...
async def page_get_text(
    ...
    try:
        ...
        text = extract_script_value(result) or ''
    except Exception as e:
        return StructuredError(
            error_code=ErrorCode.EXECUTION_ERROR,

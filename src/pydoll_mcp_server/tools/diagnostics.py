...
def server_status(client_id: str = 'anonymous', include_clients: bool = False) -> dict[str, Any]:
    ...
    try:
        result['browsers'] = [b.summary() for b in registry.list_browsers(client_id)]
...
async def diagnostics_snapshot(client_id: str = 'anonymous', include_clients: bool = False) -> dict[str, Any]:
    registry = get_registry()
    config = get_config()
    ...
    get_trace_manager().add_event_to_active(client_id, TraceEvent(
        timestamp=time.time(), tool='diagnostics_snapshot', status='success',
        summary=f'Browsers: {len(browsers)}, tabs: {len(tabs)}',
    ))
    return {
        'success': True, 'schema_version': SCHEMA_VERSION,
        'uptime_seconds': round(get_server_state().uptime_seconds, 1),
        'auth_mode': 'token' if config.auth_enabled else 'none',
        'browsers': [{'browser_id': b.browser_id, 'headless': b.headless, 'health': b.health.value,
                      'tabs': len(b.tabs)} for b in browsers],
...
async def trace_start(client_id: str, name: str = '', include_screenshots: bool = False) -> dict[str, Any]:
    ...
    return {'success': True, 'trace_id': trace.trace_id, 'name': trace.name,
            'include_screenshots': include_screenshots, 'events_count': len(trace.events)}


async def trace_stop(client_id: str, trace_id: str) -> dict[str, Any]:
...
async def trace_get(client_id: str, trace_id: str, max_events: int = 200) -> dict[str, Any]:
    trace = get_trace_manager().get(client_id, trace_id)
    if trace is None:
        return _not_found('Trace', trace_id, client_id)
    events = trace.events[-max_events:] if max_events > 0 else trace.events
    return {'success': True, 'trace_id': trace_id, 'status': trace.status,
            'events': [e.to_dict() for e in events], 'count': len(events), 'total': len(trace.events)}


async def trace_cleanup(client_id: str, older_than_seconds: int = 86400) -> dict[str, Any]:
    return {'success': True, 'cleaned': get_trace_manager().cleanup(client_id, older_than_seconds)}


async def browser_attach(client_id: str, browser_id: str) -> dict[str, Any]:
    try:
    ...
    return StructuredError(
        ErrorCode.UNSUPPORTED, 'Browser attach is not supported after server restart.',
        recovery_hint='Use browser_launch to create a new browser instance.',
    ).to_dict()


def _not_found(kind: str, resource_id: str, client_id: str) -> dict[str, Any]:

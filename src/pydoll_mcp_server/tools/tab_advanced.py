...
async def tab_new(client_id: str, browser_id: str, url: str = '') -> dict[str, Any]:
    try:
        browser = get_registry().get_browser(client_id, browser_id)
        tab = await browser._pydoll_browser.new_tab(url=url)
...
async def tab_duplicate(client_id: str, tab_id: str) -> dict[str, Any]:
    try:
        tab, browser = get_registry().resolve_tab_with_browser(client_id, tab_id)
        return await tab_new(client_id, browser.browser_id, await get_tab_url(tab._pydoll_tab))
    except StructuredError as exc:
        return exc.to_dict()


async def tab_health_check(client_id: str, tab_id: str, timeout: float = 5.0) -> dict[str, Any]:
    try:
        info = get_registry().get_tab(client_id, tab_id)
        async def probe() -> tuple[str, str]:
            url = await get_tab_url(info._pydoll_tab)
            title = await get_tab_title(info._pydoll_tab)
            await info._pydoll_tab.execute_script('return document.readyState', return_by_value=True)
            return url, title
        url, title = await asyncio.wait_for(probe(), min(timeout, 15.0))
        info.health = ResourceHealth.HEALTHY
        return {'success': True, 'tab_id': tab_id, 'health': 'healthy', 'responsive': True,
                'url': url, 'title': title, 'recommendation': 'none'}
    except StructuredError as exc:
        return exc.to_dict()
    except Exception as exc:
        return {'success': True, 'tab_id': tab_id, 'health': 'unhealthy', 'responsive': False,
                'error': str(exc), 'recommendation': 'Use tab_recover, then tab_recreate if required.'}


async def tab_recreate(client_id: str, tab_id: str, force: bool = False, preserve_url: bool = True) -> dict[str, Any]:
    if not force:
    ...
    try:
        ...
        url = await get_tab_url(info._pydoll_tab) if preserve_url else ''
        async with tab_operation_lock(tab_id):
            new_tab = await browser._pydoll_browser.new_tab(url=url)
            old_tab = info._pydoll_tab
            info._pydoll_tab = new_tab
            info.url = url
            ...
            with contextlib.suppress(Exception):
                await old_tab.close()
        return {'success': True, 'tab_id': tab_id, 'recreated': True, 'preserved_url': bool(url),
                'state_lost': ['dom', 'element_ids', 'snapshots', 'form_state']}
    except StructuredError as exc:
        return exc.to_dict()
    except Exception as exc:
...
async def dialog_list(client_id: str, tab_id: str) -> dict[str, Any]:
    try:
...
async def dialog_handle(
    client_id: str, tab_id: str, action: str = 'accept', prompt_text: str = '',
) -> dict[str, Any]:
...
async def popup_prepare(client_id: str, tab_id: str, timeout: float = 30.0) -> dict[str, Any]:
    try:
        info = get_registry().get_tab(client_id, tab_id)
...
async def popup_wait(client_id: str, watch_id: str, timeout: float = 30.0) -> dict[str, Any]:
    try:
        ...
        async def wait() -> dict[str, Any]:
            while True:
                ...
                known_objects = {item._pydoll_tab for item in get_registry().list_tabs(client_id, source.browser_id)}
                for pydoll_tab in await browser._pydoll_browser.get_opened_tabs():

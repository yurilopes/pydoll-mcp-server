...
async def download_prepare(client_id: str, tab_id: str, timeout: float = 60.0) -> JsonObject:
    try:
...
async def download_wait(client_id: str, watch_id: str, timeout: float = 60.0) -> JsonObject:
    try:
        ...
        raw_path: object = getattr(download, 'file_path', '')
...
async def download_list(client_id: str, tab_id: str) -> JsonObject:
    try:
        get_registry().get_tab(client_id, tab_id)
        items = get_watch_manager().downloads(client_id, tab_id)
        download_values: JsonArray = []
        for item in items:
            download_values.append(item)
        return {'success': True, 'downloads': download_values, 'count': len(download_values)}
    except StructuredError as exc:
        return exc.to_dict()


async def download_get_info(client_id: str, tab_id: str, name: str) -> JsonObject:
    result = await download_list(client_id, tab_id)
    for item_value in get_array(result, 'downloads', []):
        item = require_json_object(item_value, 'download item')
...
async def page_print_pdf(
    ...
    try:
        ...
        data = await tab.print_to_pdf(
            ...
            scale=scale,
            as_base64=as_base64 or not safe_path,
        )

...
@pytest.mark.asyncio
async def test_user_agent_get_reads_navigator_user_agent(monkeypatch) -> None:
...
@pytest.mark.asyncio
async def test_viewport_get_reads_window_metrics(monkeypatch) -> None:
    from pydoll_mcp_server.browser import cdp_helpers

    viewport = {'width': 1024, 'height': 768, 'device_pixel_ratio': 1.25}

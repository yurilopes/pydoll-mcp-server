...
class _FakeDownloadTab:
    def __init__(self) -> None:
        self.keep_file_at: Path | None = None
        self.timeout: float | None = None

    def expect_download(self, keep_file_at: Path, timeout: float | None = None) -> _DownloadContext:
...
def _reset_config(monkeypatch: MonkeyPatch, tmp_path: Path) -> None:
...
@pytest.mark.asyncio
async def test_download_expect_uses_runtime_download_dir(monkeypatch: MonkeyPatch, tmp_path: Path) -> None:
    ...
    assert tab.keep_file_at is not None
    assert tab.keep_file_at.resolve(strict=False) == expected_dir.resolve(strict=False)
    assert Path(string_at(result, 'path')).resolve(strict=False).is_relative_to(expected_dir.resolve(strict=False))


@pytest.mark.asyncio
async def test_upload_files_rejects_outside_allowlist(monkeypatch: MonkeyPatch, tmp_path: Path) -> None:
    from pydoll_mcp_server.tools import files as file_tools
...
@pytest.mark.asyncio
async def test_upload_files_rejects_allowed_dir_traversal(monkeypatch: MonkeyPatch, tmp_path: Path) -> None:
    from pydoll_mcp_server.tools import files as file_tools
